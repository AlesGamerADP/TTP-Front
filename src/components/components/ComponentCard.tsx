...
export function ComponentCard({ component, onClick }: ComponentCardProps) {
  ...
  const ariaLabel = `Ver detalles de componente ${component.modelo}, serie ${component.serial}, estado ${STATUS_LABELS[component.estado]}`;
    ...
    >
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            <h3 className="font-medium leading-none">{component.modelo}</h3>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center text-sm text-muted-foreground">
          ...
          Ingreso: {new Date(component.fecha_ingreso).toLocaleDateString('es-ES')}
        </div>
        
        {component.observaciones && (
          <div className="flex items-start text-sm text-muted-foreground">
            <FileText className="w-4 h-4 mr-2 mt-0.5 flex-shrink-0" />
            <p className="line-clamp-2">{component.observaciones}</p>
          </div>
        )}
